...
func (app App) router() *mux.Router {
	...
	r.HandleFunc("/v/{hashID:[a-zA-Z0-9]{3}}", app.view).Methods(http.MethodGet)
	r.HandleFunc("/{hashID:[a-zA-Z0-9]{3}}", app.redirect).Methods(http.MethodGet)
	...
	a.HandleFunc("/r/{hashID:[a-zA-Z0-9]{3}}", app.remove).Methods(http.MethodGet)
